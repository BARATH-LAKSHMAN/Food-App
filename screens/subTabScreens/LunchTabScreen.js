...
const LunchTabScreen = () => {
  ...
  useEffect(() => {
    ...
    setIsLunchTime(currentHour >= 12 && currentHour < 17);
      ...
      const fetchDishes = async () => {
        try {
          ...
          const fetchedDishes = querySnapshot.docs
            .map(doc => doc.data())
            .filter(dish => dish.categories.includes("Lunch")) // Filter for Lunch dishes
            .map(dish => ({
              ...dish,
              imageKey: dish.imageUrl // Assuming imageUrl is correctly formatted
            }));
          
          setLunchDishes(fetchedDishes);
        } catch (error) {
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="bg-primary flex-1">
        {isLunchTime ? (
          <View className="flex-row justify-evenly mb-5 flex-wrap">
            {lunchDishes.map((dish, index) => (
              <View key={index} className="w-36 h-36 mt-6 bg-veg rounded-xl shadow-lg shadow-maroon overflow-hidden">
                <TouchableOpacity onPress={() => navigation.navigate("KitchenFood", { name: dish.name, category: "Lunch"})}>
          ...
          <View className="flex-1 justify-center items-center mt-5">
            <Text className="text-black text-xl font-bold">Lunch items unavailable now</Text>
          </View>
        )}
      </ScrollView>
      <Basket />
