...
const DinnerTabScreen = () => {
  ...
  useEffect(() => {
    // Check if the current time is between 5 PM and 11 PM
    const currentHour = new Date().getHours();
    setIsDinnerTime(currentHour >= 17 && currentHour < 23);
      ...
      const fetchDishes = async () => {
        try {
          ...
          const fetchedDishes = querySnapshot.docs
            .map(doc => doc.data())
            .filter(dish => dish.categories.includes("Dinner")) // Filter for Dinner dishes
            .map(dish => ({
              ...dish,
              imageKey: dish.imageUrl // Assuming imageUrl is correctly formatted
            }));
          
          setDinnerDishes(fetchedDishes);
        } catch (error) {
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="bg-primary flex-1">
        {isDinnerTime ? (
          <View className="flex-row justify-evenly mb-5 flex-wrap">
            {dinnerDishes.map((dish, index) => (
              <View key={index} className="w-36 h-36 mt-6 bg-veg rounded-xl shadow-lg shadow-maroon overflow-hidden">
                <TouchableOpacity onPress={() => navigation.navigate("KitchenFood", { name: dish.name, category: "Dinner"})}>
          ...
          <View className="flex-1 justify-center items-center mt-5">
            <Text className="text-black text-xl font-bold">Dinner items unavailable now</Text>
          </View>
        )}
      </ScrollView>
      <Basket />
...
export default DinnerTabScreen;